...
import {
  ...
  AlertTriangle, CheckCircle2, Phone, ChevronDown, ChevronUp,
  Users, Activity, Trash2,
} from 'lucide-react';
import apiClient from '../../services/axiosInstance';
import WaitlistRegistrationModal from '../../components/waitlist/WaitlistRegistrationModal';
import { getQueue, removeFromQueue } from '../../services/receptionistService';
...
function WaitlistCard({ entry, index, onCall, onDone, onRemove, onEdit }) {
          ...
          <div className="flex items-center gap-1.5 flex-wrap justify-end">
            {entry.status === 'waiting' && (
              <button onClick={() => onCall(entry)}
                ...
                <CheckCircle2 className="w-3 h-3" /> Call
              </button>
            )}
            {entry.status === 'called' && (
              <button onClick={() => onCall(entry)}
...
function EditModal({ entry, onClose, onSaved }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await apiClient.put(`/receptionist/waitlist/${entry.id}`, payload);
      ...
      {queueLoading ? (
        ...
        <div className="text-center py-10 text-gray-400">
          ...
          <p className="text-xs mt-1 text-gray-300">
            Patients appear here after check-in.
          </p>
        </div>
      ) : (
...
export default function ReceptionistWaitlist() {
  ...
  const fetchWaitlist = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiClient.get('/receptionist/waitlist', { params });
      setEntries(res.data.data  || []);
      setMeta(res.data.meta     || { total: 0, waiting: 0, in_service: 0, done: 0 });
    } catch {
  ...
  const fetchQueue = useCallback(async () => {
    try {
      ...
      // silent — queue might just be empty
    } finally {
  ...
  // ── Initial queue load + polling every 20 seconds ─────────
  useEffect(() => {
    fetchQueue();
  ...
  const handleCall = async (entry) => {
    const nextStatus = entry.status === 'waiting' ? 'called' : 'in_service';
    try {
      await apiClient.post(`/receptionist/waitlist/${entry.id}/call`);
      setEntries((prev) => prev.map((e) => e.id === entry.id ? { ...e, status: nextStatus } : e));
      showToast(`${entry.name} ${nextStatus === 'called' ? 'called' : 'in service'}.`);
    } catch {
  ...
  const handleDone = async (entry) => {
    try {
      await apiClient.put(`/receptionist/waitlist/${entry.id}`, { status: 'done' });
  ...
  const handleRemove = async (entry) => {
    ...
    try {
      await apiClient.delete(`/receptionist/waitlist/${entry.id}`);
  ...
  const handleSaved = (updated) => {
    ...
    fetchWaitlist();
    // Also refresh queue since a new check-in may have happened
    fetchQueue();
